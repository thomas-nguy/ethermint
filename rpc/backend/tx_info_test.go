...
func (suite *BackendTestSuite) TestCreateAccessList() {
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() (evmtypes.TransactionArgs, rpctypes.BlockNumberOrHash) {
				...
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
				RegisterEthCallAny(queryClient, 100, "")
		...
		},
		{
			...
			malleate: func() (evmtypes.TransactionArgs, rpctypes.BlockNumberOrHash) {
				...
				queryClient := suite.backend.queryClient.QueryClient.(*mocks.EVMQueryClient)
				RegisterEthCallAny(queryClient, 0, "error")
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.SetupTest()
			suite.SetupMockCLient()

			args, blockNumOrHash := tc.malleate()

			suite.Require().True(blockNumOrHash.BlockNumber != nil || blockNumOrHash.BlockHash != nil,
				"BlockNumberOrHash should have either BlockNumber or BlockHash set")

			if !tc.expectError || tc.name != "error - missing from address" {
				suite.Require().NotEqual(common.Address{}, args.GetFrom(), "From address should not be zero")
			}

			result, err := suite.backend.CreateAccessList(args, blockNumOrHash, nil)
			...
			if tc.expectGasUsed {
				suite.Require().NotNil(result.GasUsed)
				suite.Require().Greater(uint64(*result.GasUsed), uint64(0))
			}

			if tc.expectAccList {
				suite.Require().NotNil(result.AccessList)
			}
		})
	}
}
